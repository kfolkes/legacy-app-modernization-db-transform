...
This folder contains the generated evidence documents from running `/dotnet.modernize`. Files appear here in order as the agent completes each phase.
...
| `08-deployment-plan.md` | Phase 8: Deployment Plan |

> Phase 4 (Implementation) writes source code to `../../modernized/dotnet-eshop/`, not here.
> Phase 6 (Build + Test) appears as CI status, not a doc.

To regenerate: open Copilot Chat → `/dotnet.modernize`.
