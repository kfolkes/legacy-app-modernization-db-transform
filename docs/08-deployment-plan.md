...
graph TB
    subgraph "Azure Cloud"
        ...
        subgraph "Data"
            ...
            APGFS[Azure DB for PostgreSQL<br/>Flexible Server D2s_v3<br/>Alternative ✨]
        end
    ...
    AS -.-> SLOT
    AS --> ASQL
    AS -.->|DatabaseProvider=PostgreSQL| APGFS
...
| **Database** | Azure SQL Database (General Purpose tier) with Managed Identity auth (passwordless), **or** Azure Database for PostgreSQL Flexible Server (General Purpose D2s_v3) with Entra ID auth (passwordless) — switchable via `DatabaseProvider` config |
...
- Azure SQL Database (serverless or provisioned) **or** Azure Database for PostgreSQL Flexible Server
- Azure Key Vault (secrets management)
- Azure Front Door (global load balancing + WAF)
- Azure Monitor (OpenTelemetry integration)
- Managed Identity (passwordless DB auth for both SQL Server and PostgreSQL)
...
│   OR Azure PG Flexible Server (D2s_v3) $120/mo
